...
export function EmployeeLogin({ onLogin, employees = [] }: EmployeeLoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For demo purposes, accept any password if not set, or check password if it exists
    if (employee.password && employee.password !== password) {
      setError('Invalid Password');
      return;
    }
    
    // If no password is set on employee, allow login with any password for demo
